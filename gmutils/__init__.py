...
from .utils import err, argparser, argparser_ml, serialize, deserialize, set_missing_attributes, isTrue, read_file, iter_file, read_dir, generate_file_iterator, monitor_setup, monitor, read_conceptnet_vectorfile, cosine_similarity, binary_distance, mkdirs, json_dump_gz, json_load_gz, deepcopy_list, deepcopy_dict, file_exists, dir_exists, file_timestamp, concat_from_list_of_dicts, binary_F1, iter_next, args_to_options

if verbose:  sys.stderr.write("\tLoading normalize ...\n")
from .normalize import normalize, ascii_fold, simplify_for_distance, naked_words
...
try:
    if not os.environ.get('GM_NO_NLP'):
        ...
        from .nlp import generate_spacy_data, spacy_ner, spacy_parsing, words_below_freq
